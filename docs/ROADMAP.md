...
**👉 NEXT — P15-4** · Historical ПВ xlsx Import → deals linked to directions (Milestone P1.5). Direction CRUD + lifecycle is live (P15-3, `/directions`) — next is the one-time import of the historical ПВ report into `deals`, assigned to directions.
...
- **Done:** migration `0002_youthful_sue_storm.sql` — directions gains `client_counterparty_id`/`owner_counterparty_id` (nullable, D16) + `status_changed_at/by` audit; **binding tables built now** (operator opted into the fuller scope) `direction_owner_bindings`/`direction_client_bindings` (§3.3/§3.4) + hand-written partial unique `uq_owner_mailbox_live` (M1, one active mailbox→one binding; per-wagon fan-out for shared mailboxes stays post-MVP). Pure domain `src/lib/directions/` (`lifecycle.ts` state machine — CLOSED→`completed`/SUSPENDED→`paused` over the locked CHECK; `activation.ts` **all 5 §1.3 guards** — client set, both rates confirmed, `client_rate > owner_rate` hard-block H1, ≥1 owner+client binding, mailbox-unique; `schema.ts` zod). Repository `repository.ts` (`DirectionError` 404/409/422; create/update[draft|open only]/transition[guarded on→active]/delete[draft-only]; binding CRUD; counterparty find-or-create reused from ПСЦ). API `POST /api/directions`, `PATCH/DELETE /api/directions/[id]` (PATCH branches transition-vs-edit), owner/client binding routes (+`[bindingId]` DELETE), and `POST /api/directions/resolve-rate` (reuses P15-2 `resolvePriceRate` — **suggestion only**, never writes, D16). UI `DirectionForm` (party pickers existing/inline, D16 client warning, ПСЦ "подтянуть" assist per side, explicit "подтвердить ставки" gate, H1 negative-margin banner), `BindingsPanel`, `StatusActions` (driven by `TRANSITIONS`), minimal `/directions` list table (replaces placeholder; rich grid → P15-5), `/directions/new` + `/directions/[id]/edit`. **Email ingestion/forwarding/worker stays P3** — only tables + binding CRUD + activation built. 30 new pure tests; type-check/lint/test(73)/build green; **live HTTP smoke all pass** (real DB): 401 unauth, 400 bad, 201 draft, activation 422 listing all unmet guards, activate 200 once client+rates+bindings present (`status_changed_*` set), `client≤owner` 422 hard margin, resolve-rate `found:false`, duplicate active mailbox 409, delete draft 200 / delete active 409.
- **Depends on:** P15-1.
