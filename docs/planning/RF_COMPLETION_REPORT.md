...
- `npx vitest run src/lib/tariff src/lib/distance --reporter=dot` → **244 passed (18 files)**.
- All **4 distance oracles** EXACT: 2444 (021609→612709), 699 (771500→648503), 3108 (023202→528706), **1432** (023202→061108 Решетниково via Ховрино, NOT via Тверь-62).
...
- Engine + seed-data byte-integrity preserved through the station-coverage and routing audits (the coverage harness was read-only and deleted; the routing detector is provably oracle-safe — see §2).
...
| **YELLOW** (computes per verbatim ТР-1 table, no kopeck oracle) | **77 probe cells** | own/rzd ПВ·ПЛ·КР·ХП·ХМ·ХЗ·ХЦ·ДМ·ОК class-2/3 + ПЛ/КР class-1 + all общий-парк (И+В) + цистерна per-tonne + рефрижератор (AMBER) + контейнер (A+B×KL ×1.05). Each emits a positive number with the correct yellow warning chain — none silently green, none fabricated. |
| **RED** (no primary-source data → no number) | **6 probe cells** | Транспортёр `ТР/{own,rzd}/{1,2,3}` — all 4572 `rateRub` cells are `null`; engine returns `total:0, confidence:red`. It refuses to price rather than invent. |
...
|---|---|---|
| Транспортёр N39–N74 (all classes, own+rzd) | 4572 `rateRub:null` + степень-негабаритности headers + belt boundaries | Прил.N2 схемы N39–N74 (sudact robot-blocked → verbatim transcription) |
...
- The remaining mainland station gaps (ЖД Якутии АЯМ, километровые halts) are derivable from Книга-3 АЯМ + the CSV «Транзитные пункты» column — fixable with no invented numbers.
...
- **55 yellow breadth rows + 77 yellow probe cells**: КР (own), all общий-парк (И+В) universal, specialized хоппер/думпкар/окатыш, цистерна per-tonne общий-парк. Highest-leverage: an R-Тариф quote for КР-own and any общий-парк universal case promotes 6+ YELLOW cells to GREEN with **zero new scraping** (belts already on disk).
- **Reefer N30/N31 (254 cells, AMBER)**: hand-verify kopeck-by-kopeck → GREEN.
- **Container +5% 2026 indexation**: pin the registered ФАС order → container loaded YELLOW → GREEN.
...
- Транспортёр N39–N74 rate plates (robot-blocked → verbatim transcription).
- Container empty-leg + thermal + контрейлер base schemes.
- Malodeyatelny per-line registry (Приказ Минтранс 313/2024 / Распоряжение РЖД 28/р — internal).
- **All CIS / Baltic / foreign + Калининград + Crimea** — OUT of scope this run.

---

## 5. The honest one-liner

**RF is NOT "полностью 1:1" yet — and this report does not claim it is.** What IS certified to the kopeck: the own-ПВ/ПЛ/ЦС + inventory-ПВ tariff block (17 oracles) and the 4 distance oracles, including the обходной fix (Решетниково 1432). What is **computed-uncertified (YELLOW)**: the broad own/общий-парк universal + specialized + reefer + container matrix — pending R-Тариф batch certification via `rf-validation-matrix.json`. What is **RED**: transporter, container empty/thermal/контрейлер, and the directional/malodeyatelny узел registry — blocked on non-public data, emitting no number. Station coverage is ~97.7 % of RF-mainland; the residual is the Якутия АЯМ + non-tariff halts (in-scope, derivable) and the structurally out-of-scope exclaves (flagged).
