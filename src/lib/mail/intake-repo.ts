...
export async function saveInboundInvoice(input: InvoiceSaveInput): Promise<{ id: string }> {
  ...
  const rows = await db
    ...
    .values({
      ...
      vatRate: numStr(input.vatRate ?? null),
      vatIncluded: input.vatIncluded ?? null,
      serviceDescription: input.serviceDescription ?? null,
      supplierKpp: input.supplierKpp ?? null,
      supplierAccount: input.supplierAccount ?? null,
      supplierBankBic: input.supplierBankBic ?? null,
      supplierCorrAccount: input.supplierCorrAccount ?? null,
      supplierBankName: input.supplierBankName ?? null,
      contractNumber: input.contractNumber ?? null,
      contractDate: toDate(input.contractDate ?? null),
