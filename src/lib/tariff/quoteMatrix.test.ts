...
describe("buildMatrixCells — инвентарный парк (N8 груж. + порожний 25(1) + В4 − 754)", () => {
  // ENGINE FIX 3: rebuilt to the CERTIFIED R-Тариф structure, validated to the kopeck against
  // the real inventory oracles (INV-1=110170 / INV-6_20=105804 @ 1409 км). The @1367 anchors
  // below are this certified formula evaluated at 1367 км (regression-lock for that distance).
  it("группа 6–20: обычный 70т = 103125 ₽, инновац 75т = 104128 ₽", () => {
    const row = byBand("6-20");
    expect(row.classic.inventoryNoVat).toBe(103125);
    expect(row.innovative.inventoryNoVat).toBe(104128);
  });

  it("повагонная (1): обычный 70т = 107500 ₽, инновац 75т = 108548 ₽", () => {
    const row = byBand("1");
    expect(row.classic.inventoryNoVat).toBe(107500);
    expect(row.innovative.inventoryNoVat).toBe(108548);
  });

  it("инвентарный > собственного (добавлены порожний + вагонная составляющая)", () => {
    for (const r of rows) {
...
describe("buildMatrixCells — ставка предоставления = инвентарный × коэффициент + НДС", () => {
  it("предоставление = round(инвентарный × 1,15)", () => {
    const row = byBand("6-20");
    expect(row.classic.provisionNoVat).toBe(Math.round(103125 * COEFF));
    expect(row.innovative.provisionNoVat).toBe(Math.round(104128 * COEFF));
  });

  it("с НДС = round(× 1,22) для инвентарного и предоставления", () => {
    const row = byBand("6-20");
    expect(row.classic.inventoryWithVat).toBe(Math.round(103125 * 1.22));
...
describe("computeInventory — обобщение на роды вагона (N8 + порожний + В)", () => {
  it("ПВ через computeInventory совпадает с ПВ-путём матрицы (yellow, число выдаётся)", () => {
    // ПВ: N8 груж. (0,77×0,909) + порожний 25(1) + В4 − 754 — тот же якорь @1367, что и матрица.
    const inv = computeInventory("ПВ", 70, DIST, 6, invData);
    expect(inv.confidence).toBe("yellow");
    expect(inv.inventoryNoVat).toBe(103125);
    expect(inv.redReason).toBeNull();
  });

  it("ПЛ: N8 груж. + порожний 25(1) + В1 — yellow, инвентарный < ПВ (В1 < В4)", () => {
    // И-часть (груж.+порожний) идентична ПВ (та же сетка N8, нерудный п.1.5 0,909 — и для платформ),
    // отличается только вагонная составляющая В1 vs В4 @1367 → ПЛ дешевле ПВ.
    ...
    expect(pl.vComponent).toBe(13191);
    expect(pl.vComponent).toBeLessThan(pv.vComponent ?? Infinity);
